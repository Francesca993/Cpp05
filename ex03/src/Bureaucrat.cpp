...
/*   Updated: 2026/02/11 16:38:31 by francesca        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeAForm(AForm const& aform) const {
    try {
        aform.execute(*this);
        std::cout << GREEN << this->getName() << " execute " << aform.getName() << RESET << std::endl;
    }
    catch (std::exception &e)  {
        std::cout << RED << this->getName() << " couldn’t execute " << aform.getName() << " because: " << e.what() << RESET << std::endl;
    }
}
